package com.chasmlabs.automation.controller.transaction;

import com.chasmlabs.automation.commons.ApiManager;
import com.chasmlabs.automation.controller.auth.LoginManager;
import com.chasmlabs.automation.dto.common.response.GetAssetMasterResponseDto;
import com.chasmlabs.automation.dto.transaction.response.TransactionResponseDto;
...
@Slf4j
public class TransactionManager extends ApiManager {
    ...
    public TransactionManager(String loginPath){
    ...
    public TransactionManager executeApi(){
        ...
        response=execute(queryParams,pathParams,headerList,null, null,TRANSACTION_URL);
        if (response.getStatusCode() != 200) {
            log.error(this.getClass() + "Coupon creation failed ... " + response.prettyPrint());
        }else {
            responseDTO = convertFromJson(response.asString(), TransactionResponseDto.class);
        }
        return this;
    }
    @Override
    ...
    @Override
    public TransactionResponseDto getResponsePojo() {
