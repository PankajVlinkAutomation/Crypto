...
import com.chasmlabs.automation.dto.primetrust.response.GetRefrenceContributionResponseDto;
...
@Slf4j
public class GetRefrenceContributionManager extends ApiManager {

    private Response response;
    private GetRefrenceContributionResponseDto responseDTO;
    private final List<Header> headerList = new ArrayList<>();
    Map<String, String> queryParams = new HashMap<>();
    Map<String, String> pathParams = new HashMap<>();

    public GetRefrenceContributionManager(String loginPath){
    ...
    public GetRefrenceContributionManager executeApi(){
        headerList.add(new Header(CLIENT_ID_HEADER,CLIENT_ID));
        headerList.add(new Header(CLIENT_SECRET_HEADER,CLIENT_SECRET));

        response=execute(queryParams,pathParams,headerList,null, null,GET_REFRENCE_CONTRIBUTION);
        if (response.getStatusCode() != 200) {
            log.error(this.getClass() + "Coupon creation failed ... " + response.prettyPrint());
        }else {
            responseDTO = convertFromJson(response.asString(), GetRefrenceContributionResponseDto.class);
        }
        return this;
    }
    @Override
    ...
    @Override
    public GetRefrenceContributionResponseDto getResponsePojo() {
