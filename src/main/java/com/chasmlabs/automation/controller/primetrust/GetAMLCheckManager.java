...
import com.chasmlabs.automation.dto.primetrust.response.GetAMLCheckResponseDto;
...
@Slf4j
public class GetAMLCheckManager extends ApiManager {

    private Response response;
    private GetAMLCheckResponseDto responseDTO;
    private final List<Header> headerList = new ArrayList<>();
    Map<String, String> queryParams = new HashMap<>();
    Map<String, String> pathParams = new HashMap<>();

    public GetAMLCheckManager(String loginPath){
    ...
    public GetAMLCheckManager executeApi(){
        headerList.add(new Header(CLIENT_ID_HEADER,CLIENT_ID));
        headerList.add(new Header(CLIENT_SECRET_HEADER,CLIENT_SECRET));

        response=execute(queryParams,pathParams,headerList,null, null,GET_AML_CHECK);
        if (response.getStatusCode() != 200) {
            log.error(this.getClass() + "Coupon creation failed ... " + response.prettyPrint());
        }else {
            responseDTO = convertFromJson(response.asString(), GetAMLCheckResponseDto.class);
        }
        return this;
    }
    @Override
    ...
    @Override
    public GetAMLCheckResponseDto getResponsePojo() {
