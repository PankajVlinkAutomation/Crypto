...
import com.chasmlabs.automation.dto.primetrust.response.GetCIPtatusResponseDto;
...
@Slf4j
public class GetCIPStatusManager extends ApiManager {

    private Response response;
    private GetCIPtatusResponseDto responseDTO;
    private final List<Header> headerList = new ArrayList<>();
    Map<String, String> queryParams = new HashMap<>();
    Map<String, String> pathParams = new HashMap<>();

    public GetCIPStatusManager(String loginPath){
    ...
    public GetCIPStatusManager executeApi(){
        headerList.add(new Header(CLIENT_ID_HEADER,CLIENT_ID));
        headerList.add(new Header(CLIENT_SECRET_HEADER,CLIENT_SECRET));

        response=execute(queryParams,pathParams,headerList,null, null,GET_CIP_STATUS);
        if (response.getStatusCode() != 200) {
            log.error(this.getClass() + "Coupon creation failed ... " + response.prettyPrint());
        }else {
            responseDTO = convertFromJson(response.asString(), GetCIPtatusResponseDto.class);
        }
        return this;
    }
    @Override
    ...
    @Override
    public GetCIPtatusResponseDto getResponsePojo() {
