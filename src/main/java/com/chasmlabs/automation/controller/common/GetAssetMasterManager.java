...
@Slf4j
public class GetAssetMasterManager extends ApiManager {

    private Response response;
    private GetAssetMasterResponseDto responseDTO;
    private final List<Header> headerList = new ArrayList<>();
    Map<String, String> queryParams = new HashMap<>();
    Map<String, String> pathParams = new HashMap<>();

    public GetAssetMasterManager(String loginPath){
    ...
    public GetAssetMasterManager executeApi(){
       ...
       // pathParams.put("whereColumn","digital_asset_name,digital_asset_ticker");
        ...
        response=execute(queryParams,pathParams,headerList,null, null,GET_ASSET_MASTER_URL);
        if (response.getStatusCode() != 200) {
            log.error(this.getClass() + "Coupon creation failed ... " + response.prettyPrint());
        }else {
            responseDTO = convertFromJson(response.asString(), GetAssetMasterResponseDto.class);
        }
        return this;
    }
    @Override
    ...
    @Override
    public GetAssetMasterResponseDto getResponsePojo() {
